...
const UpdateProductModal = ({ product }: { product: IProduct & { key: string } }) => {
  ...
  const onSubmit = async (data: FieldValues) => {
    const payload = { ...data };
    // The backend expects price and stock to be numbers, but form inputs can return strings.
    // This ensures that if these values are present, they are converted to numbers before
    // being sent to the server, preventing validation errors.
    if (payload.price) {
